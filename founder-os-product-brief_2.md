...
A chat-based intake that builds a structured founder profile by asking targeted questions:
...
Upon clicking a track in the roadmap for the first time, the founder is taken to a minimalist track set-up page, containing just a list of modules that the user can request help with. This should be checklist-style, where the user can pick and choose what they want to do for each module.

Once selected, the AI can propose a planning list, outlining what modules to cover and a brief summary of the process and outcome. The user can either accept, go back, or edit the planner directly.

The next screen is the **Track Workspace**, which contains:

**The Planner**
The planner is a two-page interface with a 'Plan' and 'Build' section. The Plan section should have everything from the Planner; the Build section should contain a list of suggestions on to-do's made by the AI, which the user can click to begin a new chat.

**Chats**
The user can start an AI chat for a specific module or general query related to the track. The AI pulls the relevant expert MD file for that module to provide structured, opinionated guidance. After each session, the founder can click a button to **save or update a chat summary as a local MD file** — this file is pulled automatically in the next session so the AI remembers progress.
...
**Progress Tracking**
- Based on the Plan and Build sections, the AI should create a simple progress bar
- Progress is evaluated based on the deliverables defined in the module's expert MD files
...
|---|---|---|
| Knows your startup | ✗ Starts from zero every session | ✓ Persistent company memory via saved MD summaries |
...
### In scope
- [x] Conversational onboarding with structured intake questions
- [x] Material upload and track-sorting during onboarding
- [x] Roadmap Developer MD → personalised roadmap generation
- [x] Dashboard with roadmap visual, kanban board, and sticky notes
- [x] Track landing pages (module overview + material review)
- [x] Track workspaces with per-module AI chats
- [x] Chat summary save-to-MD (local storage for session memory)
- [x] MD file editor within track documentation section
- [x] Module and track progress bars (deliverable-based)
